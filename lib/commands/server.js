...
(function() {
  ...
  setupServer = function(options) {
    ...
    app = connect().use(connect.logger('tiny')).use(connect.query()).use(connect.bodyParser()).use(middleware.fontcors()).use(middleware.mock(options)).use(middleware.velocity(options)).use(middleware.frameVersion()).use(middleware.fekit(options)).use(connect["static"](options.cwd, {
      hidden: true,
      redirect: true
