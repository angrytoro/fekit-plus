...
(function() {
  ...
  rsync = function(opts) {
    var args, _args;
    _args = ["-rzcvp", "--chmod=a='rX,u+w,g+w'", "--rsync-path='" + (opts.sudo ? 'sudo ' : '') + "rsync'", "" + opts.local, "" + opts.user + opts.host + ":" + opts.path, "" + (opts.include || ''), "" + (opts.exclude || '')];
    if (opts.port) {
      _args.push('-e \'ssh -p ' + opts.port + '\'');
    }
    if (opts.sudo && opts['temp-dir']) {
      _args.push('--temp-dir=' + opts['temp-dir']);
    }
    if (opts["delete"]) {
      _args.push('--delete');
    }
